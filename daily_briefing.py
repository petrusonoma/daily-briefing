...
def _is_japanese_article(title, link):
    """일본어 기사 여부 판별 — URL 도메인 + 제목 문자 범위로 이중 체크."""
    # 1) 일본 도메인 차단 (jp.chosun.com 등)
    JP_DOMAINS = ("jp.chosun.com", "chosun.jp", "japanese.chosun")
    if any(d in link for d in JP_DOMAINS):
        return True
    # 2) 제목에 일본어(히라가나 U+3040-309F / 가타카나 U+30A0-30FF) 포함 여부
    import unicodedata
    for ch in title:
        cp = ord(ch)
...
def _parse_rss_url(url, source_name, pool):
    """단일 RSS URL을 파싱해 기사 목록 반환 (일본 기사 자동 제외)."""
    ...
    for item in root.findall(".//item"):
        ...
        if not (title and link and "http" in link):
            continue
        # 일본 기사 필터
        if _is_japanese_article(title, link):
            skipped_jp += 1
            continue
        ...
        if len(result) >= pool:
            break
    if skipped_jp:
        print(f"       일본어 기사 {skipped_jp}건 제외")
    return result
