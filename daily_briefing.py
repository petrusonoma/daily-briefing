...
CHOSUNBIZ_RSS  = "https://biz.chosun.com/arc/outboundfeeds/rss/"
MK_RANKING_URL = "https://www.mk.co.kr/news/ranking"

# ── 조선비즈 RSS 풀 수집 ─────────────────────────────────────
def _fetch_chosunbiz_pool(pool=25):
    r = requests.get(CHOSUNBIZ_RSS, timeout=12, headers=_HEADERS)
    r.raise_for_status()
    ...
    for item in root.findall(".//item"):
        ...
        if title and link:
            result.append({"title": title, "url": link, "source": "조선비즈", "time": pub})
        if len(result) >= pool:
            break
    print(f"       조선비즈 RSS: {len(result)}건 수집")
    return result
...
# ── 매일경제 Top10 스크래핑 ─────────────────────────────────
def fetch_mk_top10():
    """매일경제 랭킹 페이지를 스크래핑해 순위 그대로 Top10 반환."""
    from bs4 import BeautifulSoup
    try:
        r = requests.get(MK_RANKING_URL, timeout=12, headers=_HEADERS)
        r.raise_for_status()
        soup = BeautifulSoup(r.content, "html.parser")

        result = []
        # 여러 셀렉터를 순서대로 시도 (페이지 구조 변경 대응)
        for selector in ["ol.ranking_list li", "ul.ranking_list li",
                         ".news_rank_list li", ".rank_list li", ".ranking li"]:
        ...
        print(f"       매일경제 Top10: {len(result)}건 수집")
        return result
    except Exception as e:
        print(f"       매일경제 Top10 오류: {e}")
        return []
